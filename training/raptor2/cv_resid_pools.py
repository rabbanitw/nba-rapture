"""gbm+hats3 CV with saved predictions: residual pools for the confidence
machinery of the new stack (leave-fold-out residuals, minutes recorded so the
report can bucket them), plus the definitive seeds-0 CV table for both sides.

Run:  python training/raptor2/cv_resid_pools.py
"""
...
def main():
    ...
    for side in ("offense", "defense"):
        ...
        for season, stamp in STAMPS.items():
            ...
            for yy, pp, mm in zip(y[te][elm], p[elm], mp[te][elm]):
                recs.append([season, float(yy - pp), float(mm)])
            print(f"[{side}] {season}: dev@10={s['dev@10']:.2f} "
                  f"tau@10={s['tau@10']:+.3f}", flush=True)
        pools[side] = recs
        dv = list(per.values())
        summary[side] = {"per_season": per,
                         "median": float(np.median(dv)),
                         "mean": float(np.mean(dv))}
        print(f"[{side}] median {np.median(dv):.2f} mean {np.mean(dv):.2f}",
              flush=True)

    np.savez_compressed(
        TD / "raptor2" / "hats3_resid_pools.npz",
        offense=np.array([[r[1], r[2]] for r in pools["offense"]]),
        defense=np.array([[r[1], r[2]] for r in pools["defense"]]),
        offense_season=np.array([r[0] for r in pools["offense"]]),
        defense_season=np.array([r[0] for r in pools["defense"]]))
    Path(TD / "raptor2" / "RESULTS_hats3_cv.json").write_text(
        json.dumps(summary, indent=1))
    print("wrote raptor2/hats3_resid_pools.npz + RESULTS_hats3_cv.json",
          flush=True)
