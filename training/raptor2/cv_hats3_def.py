"""Disjoint-seed replication of the defense gbm+hats3 CV (both sides included
for completeness: offense CV runs too).

Run:  python training/raptor2/cv_hats3_def.py --seedbase 10
"""
...
def main():
    ...
    ap.add_argument("--seedbase", type=int, default=10)
    args = ap.parse_args()
    ...
    for side in ("defense", "offense"):
        ...
        for season, stamp in STAMPS.items():
            ...
            row = {}
            for arm, Xa in (("gbm", Xf), ("+hats3", np.hstack([Xf, H]))):
                med = np.nanmedian(Xa[trn], axis=0)
                med = np.where(np.isfinite(med), med, 0.0)
                p = blend(Xa[trn], y[trn], Xa[te], med, params, rounds,
                          seeds=seeds)
                s = score_cells(y[te][elm], p[elm],
                                np.full(int(elm.sum()), season))
                row[arm] = round(float(s["dev@10"]), 2)
            res[season] = row
            print(f"[{side}] {season}: gbm {row['gbm']:5.2f} | +hats3 "
                  f"{row['+hats3']:5.2f}", flush=True)
        g = [res[s]["gbm"] for s in STAMPS]
        h = [res[s]["+hats3"] for s in STAMPS]
        print(f"[{side}] medians: gbm {np.median(g):.2f} | +hats3 "
              f"{np.median(h):.2f}  (+hats3 "
              f"{sum(b < a for a, b in zip(g, h))}W "
              f"{sum(b == a for a, b in zip(g, h))}T "
              f"{sum(b > a for a, b in zip(g, h))}L)", flush=True)
        out[side] = res
    Path(TD / "raptor2" / f"RESULTS_cv_hats3_s{args.seedbase}.json"
         ).write_text(json.dumps(out, indent=1))
    print(f"wrote raptor2/RESULTS_cv_hats3_s{args.seedbase}.json", flush=True)
