...
LABELS = {"box": "rap_box", "onoff": "rap_onoff"}
...
def select_timestamps(ts_by_src, model, modern_stride):
    ...
    need = ["538", "pbp", "nba-tracking"] if model == "box" else ["538", "wowy"]
    ok = set.intersection(*[ts_by_src[s] for s in need])
...
def main():
    ...
    models = ["box", "onoff"] if args.model == "both" else [args.model]
    for model in models:
        tss = select_timestamps(ts_by_src, model, args.modern_stride)
        print(f"\n[{model}] {len(tss)} timestamps selected")
        X, y, feat_names, meta = build(model, coll, report, tss)
        np.savez_compressed(
            outdir / f"{model}.npz", X=X, y=y,
